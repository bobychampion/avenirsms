...
exports.testCloudinaryCredentials = testCloudinaryCredentials;
 ...
 */
async function testCloudinaryCredentials(creds) {
 ...
 */
function generateSignedUploadParams(creds, folder) {
    const timestamp = Math.floor(Date.now() / 1000);
    const paramsToSign = `folder=${folder}&timestamp=${timestamp}`;
    const signature = (0, crypto_1.createHash)('sha1').update(paramsToSign + creds.apiSecret).digest('hex');
...
async function deleteCloudinaryAsset(creds, publicId) {
    const timestamp = Math.floor(Date.now() / 1000);
...
//# sourceMappingURL=cloudinaryAdapter.js.map