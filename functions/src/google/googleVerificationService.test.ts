...
import {
  ...
  verifyClassroom,
  verifyGmail,
} from './googleVerificationService';
import * as googleTokenService from './googleTokenService';
...
describe('googleVerificationService', () => {
  beforeEach(() => {
    ...
    (googleTokenService.getValidAccessToken as jest.Mock).mockResolvedValue('ya29.test-access-token');
  });
  ...
  describe('updateServiceStatus', () => {
    it('should update status to connected on success', async () => {
      ...
      await updateServiceStatus(schoolId, service, result);
    ...
    it('should handle different services independently', async () => {
      ...
      await updateServiceStatus(schoolId, 'drive', { success: true });
      await updateServiceStatus(schoolId, 'calendar', { success: false, error: 'Error' });
      
      expect(mockSet).toHaveBeenCalledTimes(2);
      expect(mockSet).toHaveBeenNthCalledWith(
        1,
        expect.objectContaining({ 'status.drive': 'connected' }),
        { merge: true }
      );
      expect(mockSet).toHaveBeenNthCalledWith(
        2,
        expect.objectContaining({ 'status.calendar': 'error' }),
        { merge: true }
      );
    });
  ...
  describe('verifyConnection', () => {
    it('should verify all enabled services', async () => {
      ...
      const result = await verifyConnection(schoolId);
      ...
      expect(mockSet).toHaveBeenCalledWith(
        expect.objectContaining({ 'status.calendar': 'connected' }),
        { merge: true }
      );
      expect(mockSet).toHaveBeenCalledWith(
        expect.objectContaining({ 'status.classroom': 'connected' }),
        { merge: true }
      );
      expect(mockSet).toHaveBeenCalledWith(
        expect.objectContaining({ 'status.gmail': 'connected' }),
        { merge: true }
      );
    ...
    it('should update lastVerifiedAt timestamp', async () => {
      ...
      await verifyConnection(schoolId);
    ...
    it('should throw error if integration does not exist', async () => {
      ...
      await expect(verifyConnection(schoolId)).rejects.toThrow(
  ...
  describe('verifyDrive', () => {
    it('should return success when API call succeeds', async () => {
      ...
      const result = await verifyDrive(schoolId);
  ...
  describe('verifyCalendar', () => {
    it('should return success when API call succeeds', async () => {
      ...
      const result = await verifyCalendar(schoolId);
  ...
  describe('verifyClassroom', () => {
    it('should return success when API call succeeds', async () => {
      ...
      const result = await verifyClassroom(schoolId);
  ...
  describe('verifyGmail', () => {
    it('should return success when API call succeeds', async () => {
      ...
      const result = await verifyGmail(schoolId);
