 ...
 * 
 * Provides GET and POST methods with automatic token injection,
 * retry logic, and rate limiting detection.
 */
 ...
 * Allows modification of request before sending
 */
 ...
 * Allows processing of response before returning
 */
...
class GoogleApiClientImpl implements GoogleApiClient {
   ...
   */
  private async tokenInjectionInterceptor(
    url: string,
    options: RequestInit
  ): Promise<{ url: string; options: RequestInit }> {
    // Get valid access token for this school
    const accessToken = await getValidAccessToken(this.schoolId);
  ...
  private async errorHandlingInterceptor(
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Google API request failed: ${response.status} ${response.statusText} - ${errorText}`
      );
    }
  ...
  private async executeRequest<T>(
    ...
    for (let attempt = 1; attempt <= this.retryConfig.maxAttempts; attempt++) {
      try {
        ...
        const backoffMs =
          this.retryConfig.initialBackoffMs *
          Math.pow(this.retryConfig.backoffMultiplier, attempt - 1);
        
        // Log retry attempt
        console.warn(
          `[GoogleApiClient] Request failed (attempt ${attempt}/${this.retryConfig.maxAttempts}), ` +
    ...
    // All retries failed
    throw new Error(
      `Google API request failed after ${this.retryConfig.maxAttempts} attempts: ${lastError?.message}`
    );
  }
