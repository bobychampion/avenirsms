 ...
 */
export async function verifyDrive(schoolId: string): Promise<ServiceVerificationResult> {
  try {
 ...
 * @returns Verification result indicating success or error
 */
export async function verifyCalendar(schoolId: string): Promise<ServiceVerificationResult> {
  try {
    const accessToken = await getValidAccessToken(schoolId);
 ...
 * @returns Verification result indicating success or error
 */
export async function verifyClassroom(schoolId: string): Promise<ServiceVerificationResult> {
  try {
    const accessToken = await getValidAccessToken(schoolId);
    ...
    const integrationSnap = await db
      ...
      .get();
    const storedScopes: string[] = integrationSnap.data()?.tokens?.scopes ?? [];
    const hasWriteScope = storedScopes.some(
      (s) => s.includes('classroom.courses') && !s.includes('readonly')
    );
    if (!hasWriteScope) {
      return {
        success: false,
        error:
          'Token only has read-only Classroom access. Please click Reconnect in Integration Settings to grant write permissions.',
      };
    }
 ...
 * @returns Verification result indicating success or error
 */
export async function verifyGmail(schoolId: string): Promise<ServiceVerificationResult> {
  try {
    const accessToken = await getValidAccessToken(schoolId);
...
export async function verifyConnection(
  ...
  const results: VerifyGoogleConnectionResponse['results'] = {};
