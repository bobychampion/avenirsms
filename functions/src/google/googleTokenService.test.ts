...
describe('googleTokenService', () => {
  ...
  describe('storeTokens', () => {
    it('should store tokens at the correct Firestore path', async () => {
      ...
      await storeTokens(schoolId, tokens);
    ...
    it('should handle different school IDs independently', async () => {
      ...
      await storeTokens('school1', tokens);
      await storeTokens('school2', tokens);
  ...
  describe('clearTokens', () => {
    ...
    it('should handle different school IDs independently', async () => {
      await clearTokens('school1');
      await clearTokens('school2');
  ...
  describe('isTokenExpired', () => {
    it('should return true for expired token', () => {
      ...
      const expiresAt = Timestamp.fromMillis(expiredTime);
    ...
    it('should return true for token expiring within 5 minutes', () => {
      ...
      const expiresAt = Timestamp.fromMillis(expiringTime);
    ...
    it('should return false for token expiring after 5 minutes', () => {
      ...
      const expiresAt = Timestamp.fromMillis(validTime);
    ...
    it('should handle edge case at exactly 5 minute boundary', () => {
      ...
      const expiresAt = Timestamp.fromMillis(boundaryTime);
    ...
    it('should handle edge case just after 5 minute boundary', () => {
      ...
      const expiresAt = Timestamp.fromMillis(justAfterBoundary);
    ...
    it('should handle very old expired tokens', () => {
      ...
      const expiresAt = Timestamp.fromMillis(veryOldTime);
  ...
  describe('Token expiration buffer property', () => {
    it('should consistently apply 5-minute buffer', () => {
      ...
      testCases.forEach(({ offset, expected }) => {
        const expiresAt = Timestamp.fromMillis(Date.now() + offset);
        expect(isTokenExpired(expiresAt)).toBe(expected);
      });
    });
  });
  ...
  describe('refreshAccessToken', () => {
    it('should refresh token and update Firestore', async () => {
      ...
      const result = await refreshAccessToken(schoolId);
    ...
    it('should throw error if integration does not exist', async () => {
      ...
      await expect(refreshAccessToken(schoolId)).rejects.toThrow(
        'No Google integration found for school nonexistent-school'
      );
    });
    ...
    it('should throw error if refresh token is missing', async () => {
      ...
      await expect(refreshAccessToken(schoolId)).rejects.toThrow(
        'No refresh token found for school school123'
      );
    });
  ...
  describe('getValidAccessToken', () => {
    it('should return cached token if valid', async () => {
      ...
      const expiresAt = Timestamp.fromMillis(Date.now() + 60 * 60 * 1000); // 1 hour from now
      ...
      const result1 = await getValidAccessToken(schoolId);
      expect(result1).toBe(accessToken);
      
      // Second call should use cache
      const result2 = await getValidAccessToken(schoolId);
    ...
    it('should refresh token if expired', async () => {
      ...
      const expiredAt = Timestamp.fromMillis(Date.now() - 1000); // Expired
      ...
      const result = await getValidAccessToken(schoolId);
    ...
    it('should refresh token if expiring within 5 minutes', async () => {
      ...
      const oldAccessToken = 'ya29.old-token';
      const newAccessToken = 'ya29.new-token';
      const expiringAt = Timestamp.fromMillis(Date.now() + 4 * 60 * 1000); // 4 minutes
    ...
    it('should throw error if integration does not exist', async () => {
      ...
      await expect(getValidAccessToken(schoolId)).rejects.toThrow(
        'No Google integration found for school nonexistent-school'
      );
    });
    ...
    it('should throw error if access token is missing', async () => {
      ...
      await expect(getValidAccessToken(schoolId)).rejects.toThrow(
        'No access token found for school school123'
      );
    });
    ...
    it('should handle different schools independently', async () => {
      ...
      const result1 = await getValidAccessToken(school1);
      const result2 = await getValidAccessToken(school2);
