...
import {
  ...
  Megaphone, X, Briefcase, UserCheck
} from 'lucide-react';

type NotifType = 'general' | 'fee_due' | 'exam' | 'attendance';

// Every non-parent, non-super_admin role — matches the role list Firestore Rules already
// treat as "staff" elsewhere in the app.
const STAFF_ROLES = ['teacher', 'admin', 'School_admin', 'accountant', 'hr', 'staff', 'librarian'];
...
export default function NotificationsManagement() {
  ...
  const [targetMode, setTargetMode] = useState<'all' | 'class' | 'student' | 'staff_all' | 'staff_one'>('all');
  ...
  useEffect(() => {
    ...
    const unsubStaff = onSnapshot(
      query(collection(db, 'users'), where('schoolId', '==', schoolId!), where('role', 'in', STAFF_ROLES)),
      snap => setStaffUsers(snap.docs.map(d => ({ uid: d.id, ...d.data() } as UserProfile)))
    );
  ...
  const filteredStaff = staffUsers.filter(u =>
    ...
    (u.displayName?.toLowerCase().includes(staffSearch.toLowerCase()) ||
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (targetMode === 'staff_all') {
        recipients = staffUsers.map(u => u.uid);
        if (recipients.length === 0) { toast.error('No staff accounts found.', { id: tid }); setSending(false); return; }
      } else if (targetMode === 'staff_one') {
        if (!targetStaffUid) { toast.error('Please select a staff member.', { id: tid }); setSending(false); return; }
        recipients = [targetStaffUid];
      }
      ...
      const targetLabel = targetMode === 'all' ? 'everyone'
        : targetMode === 'class' ? targetClass
        : targetMode === 'staff_all' ? 'all staff'
        : targetMode === 'staff_one' ? 'staff member'
        : 'student';
      toast.success(`Notification sent to ${targetLabel}!`, { id: tid });
      setTitle('');
      setBody('');
      setTargetStaffUid('');
      setStaffSearch('');
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  { id: 'staff_all', label: 'All Staff', Icon: Briefcase },
                  { id: 'staff_one', label: 'A Person', Icon: UserCheck },
                ].map(({ id, label, Icon }) => (
              ...
              {targetMode === 'staff_all' && (
                <p className="mt-2 text-[10px] text-slate-400">
                  Sends to every staff account at this school ({staffUsers.length} {staffUsers.length === 1 ? 'person' : 'people'}).
                </p>
              )}

              {targetMode === 'staff_one' && (
                <div className="mt-2 relative">
                  <input
                    type="text"
                    placeholder="Search staff by name or email…"
                    value={staffSearch}
                    onChange={e => { setStaffSearch(e.target.value); setTargetStaffUid(''); }}
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm outline-none focus:ring-2 focus:ring-indigo-500"
                  />
                    ...
                    <div className="absolute z-10 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-lg max-h-48 overflow-y-auto">
                      {filteredStaff.map(u => (
                        <button key={u.uid} type="button"
                          onClick={() => { setTargetStaffUid(u.uid); setStaffSearch(u.displayName || u.email); }}
                          className="w-full text-left px-4 py-2.5 hover:bg-indigo-50 text-sm font-medium text-slate-900 flex justify-between items-center">
                  ...
                  {targetStaffUid && (
                    <div className="mt-2 flex items-center gap-2 px-3 py-2 bg-indigo-50 rounded-xl border border-indigo-100">
                      <CheckCircle2 className="w-4 h-4 text-indigo-600 shrink-0" />
                      <span className="text-xs font-bold text-indigo-700">{staffSearch} selected</span>
                      <button type="button" onClick={() => { setTargetStaffUid(''); setStaffSearch(''); }}
                        className="ml-auto text-indigo-400 hover:text-indigo-600"><X className="w-3 h-3" /></button>
                    </div>
                  )}
                </div>
