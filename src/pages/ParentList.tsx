...
function generateTempPassword(name?: string): string {
  const base = (name?.split(' ')[0] || 'Parent').replace(/[^a-zA-Z]/g, '') || 'Parent';
...
// ─── Set Password Modal ────────────────────────────────────────────────────
// Lets an admin set a parent's password directly, for cases where the parent
// can't be reached at the email on file. Different from the "Reset Password"
// button, which sends a self-service reset link to that email.

function SetPasswordModal({ parent, onClose }: { parent: ParentRow; onClose: () => void }) {
  const [password, setPassword] = useState(() => generateTempPassword(parent.displayName));
  ...
  const handleSet = async () => {
    ...
    setSaving(true);
    try {
      await callApi('/api/set-parent-password', { targetUid: parent.uid, newPassword: password });
      setDone(true);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <h2 className="font-bold text-slate-900 flex items-center gap-2">
            <Lock className="w-4 h-4 text-indigo-500" /> Set Password
          </h2>
        ...
        <div className="p-5 space-y-4">
          <p className="text-sm text-slate-500">
            Directly set a new password for <span className="font-semibold text-slate-800">{parent.displayName || parent.email}</span>.
            They'll be asked to change it on next sign-in.
          </p>
          ...
          {!done ? (
            <>
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm font-mono focus:ring-2 focus:ring-indigo-500 outline-none"
                />
                <p className="text-xs text-slate-400 mt-1">At least 8 characters. Auto-generated — edit if you'd like.</p>
              </div>
              <button
                onClick={handleSet}
                disabled={saving}
            ...
            <>
              <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3 flex items-center justify-between gap-2">
                <span className="font-mono text-sm text-emerald-800">{password}</span>
                <button onClick={handleCopy} className="p-1.5 hover:bg-emerald-100 rounded-lg transition-colors shrink-0" title="Copy">
                  {copied ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-4 h-4 text-emerald-600" />}
                </button>
              </div>
              <p className="text-xs text-slate-500">Share this with the parent directly — it won't be shown again.</p>
              <button
                onClick={onClose}
                className="w-full px-4 py-2.5 bg-slate-100 text-slate-700 rounded-xl font-semibold text-sm hover:bg-slate-200 transition-all"
              >
                Done
              </button>
            </>
              ...
              <div className="pt-4 border-t border-slate-100 space-y-2">
                <div className="flex gap-2">
                  <button
                    ...
                    title="Emails a self-service reset link to the parent"
                  >
                    <KeyRound className="w-3.5 h-3.5" /> Email Reset Link
                  </button>
                  <button
                    onClick={() => setPasswordTarget(parent)}
                    ...
                    title="Set the password directly, without email"
                  >
