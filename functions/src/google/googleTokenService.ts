 ...
 * @throws Error if Firestore write fails
 */
export async function clearTokens(schoolId: string): Promise<void> {
  const db = getFirestore();
  const integrationRef = db
    .collection('schools')
    .doc(schoolId)
 ...
 * 
 * Caches valid access tokens for up to 50 minutes to reduce Firestore reads.
 ...
 */
const refreshMutexes = new Map<string, Promise<{ accessToken: string; expiresAt: Timestamp }>>();
 ...
 */
export async function getValidAccessToken(schoolId: string): Promise<string> {
  // Check cache first
